...
(function () {
  ...
  window.editProduct = function (pid) {
    ...
    modal.innerHTML =
      ...
      + '  </div>'
      + '  <div class="pe-row"><label>판매가 <i>원 · 비우면 매입가÷0.65로 자동산출</i></label>'
      + '    <input id="pe_sell" type="number" inputmode="numeric" value="' + (p.sell != null ? p.sell : '') + '" oninput="peCalc()" placeholder="자동"></div>'
  ...
  /* 입력하는 동안 마진 실시간 미리보기 — 마진은 직접 입력하는 값이 아니라 계산 결과 */
  window.peCalc = function () {
    ...
    var sell = parseInt(document.getElementById('pe_sell').value, 10);
    if (isNaN(buy)) { box.innerHTML = '<div class="pe-warn">매입가를 넣으면 손익분기·마진이 계산됩니다</div>'; return; }
    var cost = buy + ship;
    var bep = Math.ceil(cost / 0.9 / 100) * 100;             // 수수료 10% 감안 손익분기
    var auto = Math.ceil(cost / 0.65 / 100) * 100;           // 판매가 미입력 시 자동산출
    var rec = !isNaN(sell) ? sell : auto;
    var net = Math.round(rec * 0.9 - cost);
    var mg = Math.round(net / rec * 100);
    ...
    box.innerHTML =
      ...
      + '<div class="pe-line"><span>판매가' + (isNaN(sell) ? ' <i>(자동산출)</i>' : '') + '</span><b>' + won(rec) + '</b></div>'
